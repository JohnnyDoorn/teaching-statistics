...
trace_demo <- function(width = 0.8, starts = 2, n_chains = 1,
  ...
  ggplot(df, aes(iteration, lambda, colour = chain)) +
    geom_line(linewidth = 0.3, alpha = 0.85) +
    geom_hline(yintercept = 8 / 4.5, linetype = "dashed") +
    labs(title = title, x = "iteration", y = expression(lambda)) +
    theme_minimal(base_size = 12) +
    theme(legend.position = if (n_chains > 1) "right" else "none",
