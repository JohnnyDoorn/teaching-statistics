...
```{r reg-regression-one-predictor, echo=FALSE}
source("../../_setup.R")
...
```{r reg-sensitivity, fig.align='center', fig.width=10, fig.height=5, echo=FALSE}
n <- 20
# x = 
set.seed(124)
x <- rnorm(n)
...
* Normality doesn’t matter in large samples (thank you [CLT](https://en.wikipedia.org/wiki/Central_limit_theorem)).
* Normality doesn’t matter when residuals are normally distributed.
...
```{r reg-homoscedasticity, fig.align='center', fig.width=5, fig.height=5, echo=FALSE}
...
### The data {.smaller}
```{r reg-the-data-2, fig.align='center', fig.width=8, fig.height=6, echo=FALSE}
...
```{r reg-calculate-regression-parameters, eval=TRUE}
...
```{r reg-the-slope, fig.align='center', fig.width=8, fig.height=6, echo=FALSE}
# Extra
...
```{r reg-the-slope-2, fig.align='center', fig.width=8, fig.height=6, echo=FALSE}
# Extra
...
```{r reg-the-slope-zoomed-in, echo=FALSE, fig.align='center', fig.width=8, fig.height=6, echo=FALSE}
# Extra
...
```{r reg-define-regression-equation, echo=TRUE}
...
```{r reg-predicted-values, echo=FALSE}
...
```{r reg-observed-vs-predicted, fig.height=5, fig.width=5, fig.align='center', echo=FALSE}

plot(prediction,sales, xlim = c(0,360), ylim = c(0,360),
     xlab = "Predicted Sales", ylab = "Observed Sales",
     las = 1, pch = 21, bg = "lightgreen")
abline(lm(sales~prediction), col = "red")   # slope is exactly 1 here
...
```{r reg-normality-of-residuals, fig.height=5, fig.width=5, fig.align='center', echo=FALSE}
qqnorm(error,
       main = "QQ Plot of Residuals",
       xlab = "Theoretical Quantiles",
       ylab = "Sample Quantiles",
...
```{r reg-normality-of-residuals-2, fig.height=5, fig.width=5, fig.align='center', echo=FALSE}
hist(error,
...
```{r reg-explained-variance, echo=TRUE}
r^2
```

## Explained variance visually ($n = 10$) {.smaller}

```{r reg-explained-variance-visually, fig.align='center', fig.width=15, fig.height = 7, echo=FALSE}
...
```{r reg-p-value-b1, echo=FALSE}
# tCurve() lives in ../../plotFunctionsSSR.r (sourced by _setup.R) and draws the
# density the way the NHST_Continuous applet does. showRegion = FALSE: this is a
# p-value slide, so no alpha and no purple, and the observed t plus everything
# more extreme goes black. Same slide as the correlation p-value in lecture 6.
tCurve(df.b1, sided = "two", observed = t.b1, showRegion = FALSE,
       main = "Our observed t in the null distribution")

p.b1 <- 2 * pt(abs(t.b1), df.b1, lower.tail = FALSE)
```

$$P(|t| \geq `r sprintf("%.2f", abs(t.b1))` \mid H_0) `r if (p.b1 < .001) "< .001" else sprintf("= %.3f", p.b1)`$$
...
```{r reg-so-how-many-ways-do-we-have-for-assessing-an-association, echo = TRUE}
