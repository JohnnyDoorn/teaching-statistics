...
data <- read.csv(datasetsPath("diet.csv"))
data <- round(data, 2)
colnames(data) <- c("T0", "T1", "T2")
```
...
```{r, echo=FALSE}
ssrTable(data, options = list(iDisplayLength = 7))
...
visualize_chisq_base(H = F.r, df = df, section = "upper", myLimX = c(0, 6))
```
