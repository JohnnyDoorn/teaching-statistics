...
```{r, eval=TRUE, echo=FALSE}
# The variables and z-scores we made in the previous lecture
sales         <- data$sales
...
z.airplay     <- (airplay - mean(airplay)) / sd(airplay)
```
...
Recall from lecture 3: a sample statistic has its own sampling distribution, and for many familiar statistics that turns out to be a t-distribution.
...
This shape, indexed by degrees of freedom, is what we get whenever we standardize a sample statistic this way. Today's statistic is $r$.
