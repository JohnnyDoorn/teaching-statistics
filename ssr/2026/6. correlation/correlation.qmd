...
tinydat <- data[1:5, ]
x <- tinydat$anxiety
y <- tinydat$exam_grade
myLimsX <- c(55, 95)
myLimsY <- c(30, 90)
plot(x, y, las = 1, xlab = "Anxiety (EAQ)", ylab = "Exam performance (%)", 
...
n       <- 10
allData <- read.csv(datasetsPath("2026/ExamAnxiety.csv"))
data    <- allData[1:n, c("revise", "exam_grade", "anxiety")] # first 10 students
ssrTable(data)
```

```{r, eval=TRUE, echo=FALSE}
# The variables and z-scores from earlier in the lecture
...
## These were only 10 students {.smaller}

We used 10 students so we could do the sums by hand. The psychologist actually tested **103**:

```{r, echo=TRUE}
cor(allData$exam_grade, allData$anxiety)
```

Same story, weaker: small samples wobble more than large ones. This is the value the book reports.
...
plot(anxiety, exam_grade, las = 1, pch = 21, bg = "turquoise",
     ylab = "Exam performance (%)", xlab = "Anxiety (EAQ)")
...
We just built that shape for sample means. The same trick works for any statistic we can standardize, and today's is $r$.
...
plot(function(x) dt(x, df = df), -4, 4,
     main=paste0("t-distribution (df=", df, ") with two-sided alpha"),
     cex.main = .7,
     xlab="T-value",
     ylab="",
     bty="n",
...
polygon(c(pol_left,rev(pol_left)),  c(rep(0,20), dt( rev(pol_left),  df = df)), col="red", angle=-45, density=15, lwd=2)

pol_left = seq(tp975,4,length=20)
polygon(c(pol_left,rev(pol_left)),  c(rep(0,20), dt( rev(pol_left),  df = df)), col="red", angle=-45, density=15, lwd=2)

text(-2.8,dt(tp25 , df = df),expression(paste("½",alpha," = 2.5%",sep="")),pos=3, col='red')
text( 2.8,dt(tp975, df = df),expression(paste("½",alpha," = 2.5%",sep="")),pos=3, col='red')

plot(function(x) dt(x, df = df), -4, 4,
     main=paste0("t-distribution (df=", df, ") with one-sided alpha"),
     cex.main = .7,
     xlab="T-value",
     ylab="",
     bty="n", 
...
text( 2.8,dt(tp95 , df = df),expression(paste(alpha[right]," = 5%"  ,sep="")),pos=3, col='red')
```
...
pval <- pt(t.r, df, lower.tail = FALSE)
...
text(x = t.r, y = 0.35, paste("p =", round(pval, 2)), cex = 1.2)
```
