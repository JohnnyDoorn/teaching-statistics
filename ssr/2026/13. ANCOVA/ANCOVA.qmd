...
```{r ancova-simulate-data, echo=FALSE}
# Clear memory
rm(list=ls())
source("../../_setup.R")
...
```{r ancova-define-the-model, echo=FALSE}
# Define model
...
data <- cbind(n, data)
# The data are simulated above from a fixed seed, so writing them out on every
# render keeps datasets/2026/ in sync with what the slides actually show.
write.csv(data, datasetsPath("2026/ancova-openness-nationality.csv"), row.names = FALSE)
plotData <- data
...
```{r ancova-dummies, echo=FALSE}
ssrTable(dummies)
```

## Observed group means {.subsection}

```{r ancova-observed-group-means, echo=TRUE}
...
```{r ancova-fit-group-only, echo=TRUE, }
...
```{r ancova-fit-covariate-only, echo=TRUE}
...
```{r ancova-fit-full-model, echo=TRUE}
fit <- lm(extraversion ~  nationality + openness, data); fit$coefficients
```

```{r ancova-fit-full-model-rounded, echo=FALSE}
...
```{r ancova-predictions-full-model, echo=TRUE}
fit <- lm(extraversion ~ nationality + openness, data); fit$coefficients
```
...
```{r ancova-add-grand-mean, echo=FALSE}
...
```{r ancova-visual-total-variance, echo=FALSE}
...
```{r ancova-visual-explained-by-group, echo=FALSE}
...
```{r ancova-visual-explained-by-covariate, echo=FALSE}
...
```{r ancova-explained-by-full-model, echo=FALSE}
...
```{r ancova-visual-explained-by-full-model, echo=FALSE}
...
```{r ancova-unexplained-variance, echo=FALSE}
...
```{r ancova-divide-model-sum-of-squares, echo=FALSE}
...
```{r ancova-ss-covariate-corrected, echo=TRUE}
...
```{r ancova-alpha-and-power, echo=FALSE}
fValues <- seq(0, 30, .01)
...
```{r ancova-adjusted-marginal-means, echo=FALSE}
# Add dummy variables
...
```{r ancova-adjustment-factor, echo=FALSE}
aggregate(extraversion ~ nationality, data, mean)
```
