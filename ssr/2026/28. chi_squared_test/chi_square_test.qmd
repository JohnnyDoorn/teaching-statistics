...
<a href=""><img src="../../images/qr_hngxh.png" alt="edu.nl/hngxh" /></a>
...
noMonth <- sapply(strsplit(gResults$Timestamp, split = "/"), function(x) x[[3]])
years <- sapply(strsplit(noMonth, split = " "), function(x) x[[1]])
gResults <- gResults[years == "2025", ]
...
```{r, echo=FALSE}
df <- 1
visualize_chisq_base(H = chi.squared, df = df, section = "upper", myLimX = c(0, 5))
```
...
chi.squared.yates
visualize_chisq_base(H = chi.squared.yates, df = df, section = "upper", myLimX = c(0, 5))
```
