...
gResults <- as.data.frame(gResults)[, -1 ]
...
df <- 1
curve(dchisq(x, df), from = 0, to = 20, n = 1000, bty = "n", las = 1,
...
arrows(x0 = chi.squared, x1 = chi.squared, y0 = 0 , y1 = 0.6, col = "darkred", lwd = 2)
text(x = chi.squared, y = 1.2, bquote(Chi^2 ~ " = " ~ .(round(chi.squared, 2))), cex = 1.2)
text(x = chi.squared, y = 0.9, paste("p-val =", round(pchisq(chi.squared, df), 5)), cex = 1.2)
```
