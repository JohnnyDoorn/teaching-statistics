...
ml.coffee   <- round(hours.awake + (rnorm(n, 250, 100))) ## multicollinearity
error       <- rnorm(n, 0, 20) ## error
intercept   <- 50
wakefulness <- intercept + -3 * hours.awake + 
                          0.5 * ml.coffee + 
                         0.5 * hours.awake * ml.coffee + 
                         error
...
```{r, echo=TRUE}
#fit2 <- lm(wakefulness ~ hours.awake + ml.coffee)
fit <- lm(wakefulness ~ hours.awake + ml.coffee + hours.awake*ml.coffee); summary(fit)
```
